...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "item_id", type=int, help="ID of the item to check transitions for"
        )
    ...
    def handle(self, *args, **options):
        ...
        if item.parent:
            ...
            self.stdout.write(
                f"Parent Project Status: {item.parent.get_status_display()}"
            )
        ...
        transitions = (
            item.get_available_transitions()
            if options["available_only"]
            else item.get_all_transitions()
        )
        label = (
            "Available transitions" if options["available_only"] else "All transitions"
        )
