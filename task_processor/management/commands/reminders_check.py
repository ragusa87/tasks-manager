        ...
        try:
            if options['async']:
                ...
                if options['verbose']:
                    ...
                    self.stdout.write(f"Processed at: {task_result.get('processed_at', 'Unknown')}")
                    self.stdout.write(f"Reminders sent: {task_result.get('reminders_sent', 0)}")
                    self.stdout.write(f"Errors: {task_result.get('errors', 0)}")
                    self.stdout.write(f"Total items checked: {task_result.get('total_items_checked', 0)}")
                else:
