...
class AirDatepickerMixin:
    ...
    def build_attrs(self, base_attrs, extra_attrs=None):
        ...
        attrs["data-airdatepicker"] = json.dumps(
            {
                "locale": "fr",
                "firstDay": 1,
                "dateFormat": self.date_format_js,
                "timeFormat": self.time_format_js,
...
class RecurrenceField(forms.CharField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs.setdefault(
            "help_text",
            "Optional: Enter RRULE recurrence pattern (e.g., FREQ=DAILY;INTERVAL=1 or FREQ=WEEKLY;BYDAY=MO,WE)",
        )
            ...
            if hasattr(rrule, "_freq"):
                ...
                if rrule._freq > 3:  # More frequent than daily (hourly, minutely, etc.)
                    raise ValidationError(
                        "Maximum recurrence frequency is daily. Sub-daily patterns (hourly, minutely) are not allowed."
                    )
...
class ItemForm(forms.ModelForm):
    ...
    tags = forms.CharField(
        required=False, widget=forms.HiddenInput(), help_text="Comma-separated tag IDs"
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "review_frequency_days": forms.NumberInput(
                attrs={"min": "1", "max": "365"}
            ),
